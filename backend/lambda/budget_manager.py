...
def configure_budget(event):
    ...
    try:
        ...
        if not isinstance(thresholds, list) or not all(isinstance(t, (int, float)) for t in thresholds):
    ...
    try:
        ...
        for budget in budgets:
            ...
            budget_status.append({
                ...
                'percentage_used': round((current_spending / monthly_limit * 100), 1),
                ...
                'status': 'over_budget' if current_spending > monthly_limit else 
                         'warning' if exceeded_thresholds else 'on_track'
            })
            ...
            for threshold in budget['thresholds']:
                if percentage_used >= threshold:
                    ...
                    if alert_key not in last_alerts:
                        # Send alert
                        send_budget_alert(budget, threshold, current_spending, monthly_limit)
...
def get_current_spending(service_filter='all'):
    ...
    try:
        ...
        allowed_services = ['all', 'EC2-Instance', 'Lambda', 'S3', 'DynamoDB', 'CloudFront', 'API Gateway']
        if service_filter not in allowed_services:
            logger.warning(f'Unauthorized service filter attempted: {sanitize_input(str(service_filter))}')
            service_filter = 'all'
...
def get_from_cache(cache_key):
    ...
    try:
        ...
        allowed_patterns = [r'^current_costs_\d{4}-\d{2}-\d{2}-\d{2}$', r'^cost_trends_\d{4}-\d{2}-\d{2}-\d{2}$']
        if not any(re.match(pattern, cache_key) for pattern in allowed_patterns):
            logger.warning(f'Unauthorized cache key pattern: {sanitize_input(str(cache_key))}')
            return None
